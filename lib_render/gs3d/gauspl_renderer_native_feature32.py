...
import torch
import math
from diff_gaussian_rasterization_feat32 import (
...
def render_cam_pcl(
    ...
    ret = rasterizer(
        ...
        shs=None,
        semantic_feature=semantic_feature.float(), # assign value
    ...
    if len(ret) == 5:
        rendered_image,feature_map , radii, depth, alpha = ret
    else:
    ...
    if center_handling_flag:
        for k in ["rgb", "dep", "alpha", "feature_map"]:
            if ret[k] is None:
                continue
            if left_w > right_w:
                ret[k] = ret[k][:, :, :W]
