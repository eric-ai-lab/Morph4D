...
# def eval_dycheck(
#     save_dir, gt_rgb_dir, gt_mask_dir, pred_dir, strict_eval_all_gt_flag=False
# ):
...
#     from dycheck_metrics_pt import compute_ssim as compute_ssim_pt

#     jax_device_cpu = jax.devices("cpu")[0]
...
#             gt = (
#                 imageio.imread(osp.join(gt_rgb_dir, gt_rgb_fns[i])).astype(float)
#                 / 255.0
#             )
#             gt_mask = (
#                 imageio.imread(osp.join(gt_mask_dir, gt_mask_fns[i])) > 0
#             ).astype(float)[..., None]
...
#             print(f"compute_psnr time: {time.time() - start_t:.2f}s")

#             start_t = time.time()
#             tmp_ssim = compute_ssim(gt, pred, full_mask).item()
#             print(f"compute_ssim time: {time.time() - start_t:.2f}s")

#             start_t = time.time()
#             tmp_ssim2 = compute_ssim_pt(
#                 torch.from_numpy(gt).permute(2, 0, 1)[None].float(),
#                 torch.from_numpy(pred).permute(2, 0, 1)[None].float(),
#                 torch.from_numpy(full_mask).permute(2, 0, 1)[None].float(),
#             ).item()
#             print(f"compute_ssim2 time: {time.time() - start_t:.2f}s")
#             print(abs(tmp_ssim - tmp_ssim2))

#             start_t = time.time()
#             tmp_lpips = compute_lpips(
#                 lpips_fn, gt, pred, full_mask, device=device
#             ).item()
#             print(f"compute_lpips time: {time.time() - start_t:.2f}s")
...
#             ignore_index=True,
#         )
#     df.to_excel(osp.join(save_dir, "dycheck_metrics.xlsx"), index=False)

#     viz_fns = sorted(os.listdir(save_viz_dir))
...
#     gt_rgb_dir = "../../data/iphone_1x_dev/spin/test_images/"
#     gt_mask_dir = "../../data/iphone_1x_dev/spin/test_covisible/"
#     pred_rgb_dir = "../../data/iphone_1x_dev/spin/log/native_iphone_single_ref.yaml20240507_141321/test/"
#     # save_dir = "../../data/iphone/spin/log/20240401_104110/"
#     save_dir = "../../debug/jax_gpu"
