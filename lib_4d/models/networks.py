...
class CNN_decoder(nn.Module):
    def __init__(self, input_dim, output_dim, patch_size=14):
        super().__init__()
        ...
        self.conv = nn.Conv2d(
            in_channels=input_dim, out_channels=output_dim, # 3 1408
            kernel_size=(patch_size, patch_size), # 1 14 14
            stride=(patch_size, patch_size) # 1 14 14
        )

    def forward(self, x, resolution=224):
        # resize resolution if necessary
        if x.shape[-1] != resolution: # TODO: NOTICE: this will even resize the input tensor if it is not a square - Hui
            x = F.interpolate(x[None,], size=(resolution, resolution), mode='bilinear', align_corners=False)[0]
