...
class FeatureImageFolderLoader(enc_ds.ADE20KSegmentation):#(torch.utils.data.Dataset):
    def __init__(self, student_feature_root, teacher_feature_root, image_root, eval_mode, transform=None):
        ...
        assert len(self.student_features) == len(self.teacher_features)
    ...
    def __getitem__(self, index):
        student_feature = torch.load(self.student_features[index], map_location="cpu") ### [0]
        teacher_feature = torch.load(self.teacher_features[index], map_location="cpu")
...
def test(args):
    ...
    accs=[]
    ious=[]
    student_inference_time=0
    teacher_inference_time=0
    ...
    for i, (student_features, teacher_features, images, dst) in enumerate(tbar):
        ...
        with torch.no_grad():
            ...
            student_inference_time += time.time() - tmp_start_time
            tmp_start_time = time.time()
            ...
            teacher_inference_time += time.time() - tmp_start_time
        ...
        for pred_predict, gt_predict, impathp in zip(pred_predicts, gt_predicts, dst):
            ...
            outdir = os.path.join(args.student_feature_dir, "lseg_eval_result")
            os.makedirs(outdir, exist_ok=True)
            outname = f"famp{count-1}"
            print(outname)
    ...
    save_path = os.path.join(args.student_feature_dir, 'lseg_eval_result_new.json')
    with open(save_path, 'w') as f:
        json.dump(ret, f, indent=4)
    print(f"student inference time: {student_inference_time}, teacher inference time: {teacher_inference_time}")
    with open(os.path.join(args.student_feature_dir, 'lseg_eval_time.json'), 'w') as f:
        json.dump({"student_inference_time": student_inference_time, "teacher_inference_time": teacher_inference_time}, f, indent=4)
...
if __name__ == "__main__":
    ...
    # save code backup, just this file
    code_save_dir = os.path.join(args.student_feature_dir, 'code_backup')
    os.makedirs(code_save_dir, exist_ok=True)
    shutil.copy(__file__, code_save_dir)
