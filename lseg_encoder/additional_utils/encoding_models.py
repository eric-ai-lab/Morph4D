...
class MultiEvalModule(DataParallel):
    ...
    def parallel_forward(self, inputs, **kwargs):
        ...
        """
        # Fast path: single GPU or CPU fallback – avoid NCCL/DataParallel overhead
        if not torch.cuda.is_available() or len(self.device_ids) <= 1:
            target_device = self.device_ids[0] if (hasattr(self, 'device_ids') and self.device_ids) else torch.device('cuda' if torch.cuda.is_available() else 'cpu')
            single_device_inputs = [inp.unsqueeze(0).to(target_device) for inp in inputs]
            if len(single_device_inputs) == 1:
                return [self.forward(single_device_inputs[0], **kwargs)]
            else:
                return [self.forward(inp, **kwargs) for inp in single_device_inputs]

        inputs = [(input.unsqueeze(0).cuda(device),)
                  for input, device in zip(inputs, self.device_ids)]
        replicas = self.replicate(self, self.device_ids[:len(inputs)])
        # kwargs needs to be a list with the same length as inputs for parallel_apply
        if isinstance(kwargs, list):
            kwargs_list = kwargs
        else:
            kwargs_list = [kwargs for _ in range(len(inputs))]
        outputs = self.parallel_apply(replicas, inputs, kwargs_list)
        #for out in outputs:
