...
def test(args):
    ...
    seg_path = os.path.join(args.data, "lseg_results/segmentation")
    os.makedirs(seg_path, exist_ok=True)

    for name in ["lseg_results"]:
        input_path = os.path.join(args.data, name)
        if not os.path.exists(input_path):
            print(name, "do not exists!")
            continue
        image_path = os.path.join(input_path, "renders")
        feature_path = os.path.join(input_path, "saved_features")
