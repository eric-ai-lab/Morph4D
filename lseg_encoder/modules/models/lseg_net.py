...
class LSeg_feature_seg:
    def __init__(
        ...
        self.labels = self.get_labels('ade20k')
        self.text = clip.tokenize(self.labels).cuda()
        self.text_features =  self.clip_pretrained.encode_text(self.text)
        self.text_features /= self.text_features.norm(dim=-1, keepdim=True) # torch.Size([150, 512])
        self.out_c=512
        self.logit_scale = nn.Parameter(torch.ones([]) * np.log(1 / 0.07)).exp()

    def __call__(self, img_feature):
        if len(img_feature.shape) == 3:
            img_feature = img_feature.unsqueeze(0)
        assert img_feature.shape[1] == 512
        imshape = img_feature.shape
        image_features = img_feature.permute(0,2,3,1).reshape(-1, self.out_c)
        image_features /= image_features.norm(dim=-1, keepdim=True) # torch.Size([57600, 512])

        logits_per_image = self.logit_scale * image_features.half() @ self.text_features.t() # torch.Size([57600, 150])
        out = logits_per_image.float().view(imshape[0], imshape[2], imshape[3], -1).permute(0,3,1,2) # torch.Size([1, 150, 240, 240])

        # out = self.scratch.output_conv(out) # torch.Size([1, 150, 480, 480])

        predict = out.argmax(dim=1).cpu().numpy()
        mask = utils.get_mask_pallete(predict, 'detail')
        return mask
    ...
    def get_labels(self, dataset):
        labels = []
        path = 'encoders/lseg_encoder/label_files/{}_objectInfo150.txt'.format(dataset)
...
if __name__ == "__main__":
    lseg_feat_seg = LSeg_feature_seg()
    img_feature = torch.load("/public/home/renhui/code/4d/4d-gaussian-splatting/data/dnerf/bouncingballs/rgb_feature_langseg/test/r_000_fmap_CxHxW.pt").cuda()
    mask = lseg_feat_seg(img_feature)