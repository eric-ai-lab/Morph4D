...
def main(exp_root, gt_root, point, tto):
    ...
    save_dir=osp.join(exp_root, f"{prefix}test_feat_segmentation")
    feature_config = osp.join(exp_root, "feature_config.yaml")
    feature_head_ckpt_path = osp.join(exp_root, "finetune_semantic_heads.pth")
    rendered_feature_path = osp.join(exp_root, f"{prefix}test_latent_features.pth")
    ...
    if not osp.exists(pred_seg):
        segmentation(semantic_head_path=feature_head_ckpt_path, rendered_feature_path=osp.join(exp_root, f"{prefix}test_latent_features.pth"), head_config=feature_config, point=point, video_dir=osp.join(exp_root, f"{prefix}test"),save_dir=save_dir)
