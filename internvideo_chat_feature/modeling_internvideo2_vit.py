...
class PatchEmbed(nn.Module):
    ...
    def __init__(
        ...
        self.proj = nn.Conv3d(
            in_channels=in_chans, out_channels=embed_dim, # 3 1408
            kernel_size=(tubelet_size, patch_size[0], patch_size[1]), # 1 14 14
            stride=(tubelet_size, patch_size[0], patch_size[1]) # 1 14 14
        )
...
class PretrainVisionTransformer_clean(nn.Module):
    ...
    # def get_feature(self,x,use_image=False ):
    #     pass
    
    # @torch.cuda.amp.autocast(enabled=False)
    def forward(self, x, mask=None, use_image=False,ret_map=False):
        input_w, input_h = x.shape[-2], x.shape[-1]
        x = self.patch_embed(x.type(self.dtype)) # 1,3,8,224,224
        # print(f"x.shape: {x.shape} x.dtype: {x.dtype}, model.dtype: {self.dtype}")
        B, T, L, C = x.shape  # T: temporal; L: spatial     1,8,256,1408
        x = x.view([B, T * L, C]) # 1 2048 1408
        
        # reverse back
        x_reverse = x.view([B, T, L, C])
        ...
        if ret_map:
            ...
            class_feat = x_vis[:, 0, :].unsqueeze(1)
            vid_feature = x_vis[:, 1:, :]

            # reshape vid feature back to 8*16*16
            vid_feature = vid_feature.view(1, T , input_h // 14, input_w // 14, self.embed_dim)
            return vid_feature, class_feat
